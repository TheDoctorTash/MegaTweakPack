...
set SystemUser=%~dp0superUser_%MyOS%.exe -w -c
set acl=%~dp0subinacl.exe /noverbose /nostatistic /errorlog="%logfile%" /outputlog="%logfile%"
...
if !errorlevel!==1 (
	...
	del /f /q /s "%LocalAppdata%\Microsoft\Windows\INetCache\IE\*" 1>> %logfile% 2>>&1
	...
	for /f "usebackq tokens=*" %%l in (`reg query HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VolumeCaches`) do ( reg add "%%l" /v "StateFlags0001" /t REG_DWORD /d "2" /f 1>> %logfile% 2>>&1 )
	...
	::for /f "tokens=*" %%i in ('wevtutil el') do (wevtutil cl "%%i" 1>> %logfile% 2>>&1)
	set timerEnd=!time!
	call :timer
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[91m 6)%clr%[36m Включить использование расширенного режима APIC. Предупреждение: предварительно включите x2APIC в BIOS после перезагрузки.%clr%[92m
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[91m 8)%clr%[36m Отключить ограничения памяти ядра для запуска BitLocker (вызывает сбои/циклы загрузки, если включен Intel Software Guard Extensions).%clr%[92m
@echo Отключить ограничения памяти ядра для запуска BitLocker (вызывает сбои/циклы загрузки, если включен Intel Software Guard Extensions). 1>> %logfile%
set timerStart=!time!
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[91m 2)%clr%[36m Отключить таймер событий высокой точности (High Precision Event Timer). Предупреждение: предварительно включите HPET в BIOS после перезагрузки.%clr%[92m
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[91m 5)%clr%[36m Отключить безопасный режим памяти DMA и изоляцию ядер для машин Hyper-V. Предупреждение: включение режима может привести к сбою старых приложений и драйверов или даже к BSOD.%clr%[92m
...
echo ••••••••••••
echo. 1>> %logfile%
@echo %clr%[36m Скрыть экран приветствия при входе в систему. Примечание: доступно на сборках Education и Enterprise.%clr%[92m
@echo Скрыть экран приветствия при входе в систему. Примечание: доступно на сборках Education и Enterprise. 1>> %logfile%
set timerStart=!time!
...
for /f "tokens=2 delims==" %%i in ('wmic useraccount where "name='%username%'" get SID ^| findstr "."') do (set SID=%%i)
reg add "HKLM\SOFTWARE\Microsoft\Windows Embedded\Shell Launcher\%SID%" /v "Shell" /t REG_SZ /d "%SystemRoot%\explorer.exe" /f 1>> %logfile% 2>>&1
...
reg add "HKLM\SOFTWARE\Microsoft\Windows Embedded\Shell Launcher\S-1-5-32-544" /v "Shell" /t REG_SZ /d "%SystemRoot%\explorer.exe" /f 1>> %logfile% 2>>&1
...
@echo Отключить Windows Defender Antivirus. 1>> %logfile%
set timerStart=!time!
...
%SystemUser% WdNisSvc 1>> %logfile% 2>>&1
...
powershell "Disable-WindowsOptionalFeature -Online -FeatureName Windows-Defender-Default-Definitions -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
...
@echo Отключить SmartScreen. 1>> %logfile%
set timerStart=!time!
...
@echo Удалить Microsoft Edge. 1>> %logfile%
set timerStart=!time!
...
powershell "Start-Process -FilePath '${env:ProgramFiles(x86)}\Microsoft\Edge\Application\*\setup.exe' -ArgumentList '-uninstall -system-level -verbose-logging -force-uninstall' -NoNewWindow" 1>> %logfile% 2>>&1
...
del /f /q /s "%Appdata%\Macromedia\Flashp~1">> %logfile% 2>>&1
rmdir /q /s "%Appdata%\Macromedia\Flashp~1">> %logfile% 2>>&1
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[91m 8)%clr%[36m Установить для параметра Maximum Transmission Unit (MTU) значение 1492.%clr%[92m
@echo Установить для параметра Maximum Transmission Unit (MTU) значение 1492. 1>> %logfile%
set timerStart=!time!
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[91m 20)%clr%[36m Запретить весь NTLM траффик. Предупреждение: блокировка запросов NTLM нарушает работу RDP и Samba.%clr%[92m
@echo Запретить весь NTLM траффик. Предупреждение: блокировка запросов NTLM нарушает работу RDP и Samba. 1>> %logfile%
set timerStart=!time!
...
set timerStart=!time!
powershell "Set-NetTCPSetting -SettingName Internet -MinRto 300" 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
if /i %browsersFound%==1 (
	...
	if !errorlevel!==1 (
		...
		cmd.exe /c "%~dp0\..\Tools\speedyfox.exe "/Firefox:all" "/Chrome:all" "/Microsoft Edge:all" "/Skype:all" "/Thunderbird:all" "/Opera:all" "/Vivaldi:all" "/Yandex Browser:all" "/Cyberfox:all" "/FossaMail:all" "/Pale Moon:all" "/SeaMonkey:all"" 1>> %logfile% 2>>&1
		set browsersFound=0
	)
)
if "%MyOS%"=="x64" (
...
echo ••••••••••••
echo •••••••••••• 1>> %logfile%
@echo %clr%[91m 1) %clr%[36m Отключить обновление драйверов через Центр обновления Windows.%clr%[92m
@echo Отключить обновление драйверов через Центр обновления Windows. 1>> %logfile%
set timerStart=!time!
...
set timerStart=!time!
for /f "usebackq tokens=* delims==" %%l in (`reg query "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings" /s /v "Attributes"^|FindStr HKEY_`) do ( reg add "%%l" /v "Attributes" /t REG_DWORD /d "1" /f 1>> %logfile% 2>>&1 )
...
for /f "usebackq tokens=* delims==" %%l in (`reg query "HKLM\SOFTWARE\Microsoft\WcmSvc\wifinetworkmanager\features" /s ^|FindStr HKEY_`) do ( reg add "%%l" /v "FeatureStates" /t REG_DWORD /d "828" /f 1>> %logfile% 2>>&1 )
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
call :disable_svc Sense 1>> %logfile% 2>>&1
echo. 1>> %logfile%
rem #########################################################################################################################################################################################################################
...
cmd.exe /c del /f /q C:\ProgramData\Microsoft\Search\Data\Applications\Windows\Windows.edb 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
echo. 1>> %logfile%
@echo %clr%[36m Отключить создание последней удачной конфигрурации загрузки.%clr%[92m
@echo Отключить создание последней удачной конфигрурации загрузки. 1>> %logfile%
set timerStart=!time!
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Разрешить очистку последних файлов при выходе.%clr%[92m
@echo Разрешить очистку последних файлов при выходе. 1>> %logfile%
set timerStart=!time!
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Удалить Kernel из черного списка.%clr%[92m
@echo Удалить Kernel из черного списка. 1>> %logfile%
set timerStart=!time!
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Включить гибернацию и быстрый запуск.%clr%[92m
@echo Включить гибернацию и быстрый запуск. 1>> %logfile%
set timerStart=!time!
reg add "HKLM\System\CurrentControlSet\Control\Power" /v "HibernateEnabled" /t REG_DWORD /d "1" /f 1>> %logfile% 2>>&1
reg add "HKLM\System\CurrentControlSet\Control\Power" /v "HiberFileSizePercent" /t REG_DWORD /d "10" /f 1>> %logfile% 2>>&1
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Выставить низкий уровень UAC (контроль учетных записей). Полное отключение приведет к поломке некоторых приложений.%clr%[92m
@echo Выставить низкий уровень UAC (контроль учетных записей). Полное отключение приведет к поломке некоторых приложений. 1>> %logfile%
set timerStart=!time!
...
echo. 1>> %logfile%
rem #########################################################################################################################################################################################################################
::@echo %clr%[36m Отключить файервол. После отключения возможно не будет работать файервол в сторонней антивирусной программе.%clr%[92m
::@echo Отключить файервол. После отключения возможно не будет работать файервол в сторонней антивирусной программе. 1>> %logfile%
::set timerStart=!time!
...
::@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
::echo. 1>> %logfile% 2>>&1
@echo %clr%[36m Добавление дополнительных правил файервола для усиления защиты ОС.%clr%[92m
...
@echo Установить файл Hosts от StevenBlack, блокирующий рекламу и телеметрию. 1>> %logfile%
set timerStart=!time!
...
@echo ОК %clr%[93m[%clr%[91m!mins!%clr%[0m минут%clr%[93m %clr%[91m!secs!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Добавление черного списка телеметрии в файл hosts и правила файервола.%clr%[92m
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Добавление AdGuard DoH провайдеров для фильтрации рекламы. DoH протокол обладает более высокой надёжностью по сравнению с DNSCrypt.%clr%[92m
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Установить классическое контекстное меню панели задач.%clr%[92m
@echo Установить классическое контекстное меню панели задач. 1>> %logfile%
set timerStart=!time!
xcopy %~dp0TaskbarContextMenuTweaker.dll "%SystemRoot%\System32" /c /q /h /r /y 1>> %logfile% 2>>&1
reg add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer" /v "ContextMenuConfig" /t REG_DWORD /d "1" /f 1>> %logfile% 2>>&1
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "TaskbarContextMenu" /t REG_SZ /d "rundll32 TaskbarContextMenuTweaker.dll,Inject" /f 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
%SystemUser% reg load "hku\.Default" "%HomeDrive%\Users\Default\NTUSER.DAT" 1>> %logfile% 2>>&1
reg add "HKU\.Default\Software\Microsoft\Windows\CurrentVersion\Explorer\Taskband\AuxilliaryPins" /v "MailPin" /t REG_DWORD /d "0" /f 1>> %logfile% 2>>&1
%SystemUser% reg unload "hku\.Default" 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Сделать рабочий стол быстрым.%clr%[92m
@echo Сделать рабочий стол быстрым. 1>> %logfile%
set timerStart=!time!
...
powershell "Enable-WindowsOptionalFeature -Online -FeatureName Windows-Identity-Foundation -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!mins!%clr%[0m минут%clr%[93m %clr%[91m!secs!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
set timerStart=!time!
powershell "Enable-WindowsOptionalFeature -Online -FeatureName LegacyComponents -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!mins!%clr%[0m минут%clr%[93m %clr%[91m!secs!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
set timerStart=!time!
powershell "Enable-WindowsOptionalFeature -Online -FeatureName Containers-DisposableClientVM -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!mins!%clr%[0m минут%clr%[93m %clr%[91m!secs!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Включить блокировку устройства.%clr%[92m
@echo Включить блокировку устройства. 1>> %logfile%
set timerStart=!time!
powershell "Enable-WindowsOptionalFeature -Online -FeatureName Microsoft-Windows-Client-EmbeddedExp-Package -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
powershell "Enable-WindowsOptionalFeature -Online -FeatureName Client-DeviceLockdown -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
powershell "Enable-WindowsOptionalFeature -Online -FeatureName Client-EmbeddedShellLauncher -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
powershell "Enable-WindowsOptionalFeature -Online -FeatureName Client-EmbeddedBootExp -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
powershell "Enable-WindowsOptionalFeature -Online -FeatureName Client-EmbeddedLogon -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!mins!%clr%[0m минут%clr%[93m %clr%[91m!secs!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
set timerStart=!time!
powershell "Enable-WindowsOptionalFeature -Online -FeatureName HypervisorPlatform -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
powershell "Enable-WindowsOptionalFeature -Online -FeatureName Microsoft-Hyper-V-All -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!mins!%clr%[0m минут%clr%[93m %clr%[91m!secs!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
set timerStart=!time!
powershell "Enable-WindowsOptionalFeature -Online -FeatureName DirectPlay -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!mins!%clr%[0m минут%clr%[93m %clr%[91m!secs!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
set timerStart=!time!
powershell "Enable-WindowsOptionalFeature -Online -FeatureName SMB1Protocol -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
powershell "Enable-WindowsOptionalFeature -Online -FeatureName SMB1Protocol-Client -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
powershell "Enable-WindowsOptionalFeature -Online -FeatureName SMB1Protocol-Server -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
powershell "Enable-WindowsOptionalFeature -Online -FeatureName SMB1Protocol-Deprecation -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
...
set timerStart=!time!
powershell "Disable-WindowsOptionalFeature -Online -FeatureName Printing-XPSServices-Features -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
powershell "Disable-WindowsOptionalFeature -Online -FeatureName Printing-PrintToPDFServices-Features -NoRestart -WarningAction SilentlyContinue" 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!mins!%clr%[0m минут%clr%[93m %clr%[91m!secs!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Установить кодек HEVC для воспроизведения 4К видео.%clr%[92m
@echo Установить кодек HEVC для воспроизведения 4К видео. 1>> %logfile%
set timerStart=!time!
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Отключить оптимизацию доставки P2P центра обновления Windows компьютерам в локальной сети (полное отключение приводит к ошибке при загрузке с магазина Windows).%clr%[92m
@echo Отключить оптимизацию доставки P2P центра обновления Windows компьютерам в локальной сети (полное отключение приводит к ошибке при загрузке с магазина Windows). 1>> %logfile%
set timerStart=!time!
...
call :remove_uwp Microsoft3DViewer
...
call :remove_uwp Microsoft.CredDialogHost
call :remove_uwp Microsoft.ECApp
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Отключить службу push-уведомлений по протоколу беспроводных приложений (WAP) для управления устройствами. Предупреждение: эта служба необходима для взаимодействия с Microsoft Intune.%clr%[92m
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Отключить возможности подключенного пользователя и телеметрию (ранее называвшуюся службой отслеживания диагностики), заблокировать соединение единой телеметрии через правила брандмауэра для исходящего трафика.%clr%[92m
@echo Отключить возможности подключенного пользователя и телеметрию (ранее называвшуюся службой отслеживания диагностики), заблокировать соединение единой телеметрии через правила брандмауэра для исходящего трафика. 1>> %logfile%
set timerStart=!time!
...
call :disable_task "\Microsoft\Windows\Chkdsk\SyspartRepair"
call :disable_task "\Microsoft\Windows\SettingSync\BackgroundUpLoadTask"
call :disable_task "\Microsoft\Windows\Device Setup\Metadata Refresh"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\HandleCommand"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\HandleWnsCommand"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\IntegrityCheck"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\LocateCommandUserSession"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterDeviceAccountChange"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterDeviceConnectedToNetwork"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterDeviceLocationRightsChange"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterDevicePeriodic1"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterDevicePeriodic24"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterDevicePeriodic6"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterDevicePolicyChange"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterDeviceScreenOnOff"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterDeviceSettingChange"
call :disable_task "\Microsoft\Windows\DeviceDirectoryClient\RegisterUserDevice"
...
call :disable_svc DiagTrack 1>> %logfile% 2>>&1
call :disable_svc diagnosticshub.standardcollector.service 1>> %logfile% 2>>&1
call :disable_svc dmwappushservice 1>> %logfile% 2>>&1
call :disable_svc WerSvc 1>> %logfile% 2>>&1
call :disable_svc wercplsupport 1>> %logfile% 2>>&1
call :disable_svc PcaSvc 1>> %logfile% 2>>&1
call :disable_svc wisvc 1>> %logfile% 2>>&1
call :disable_svc RetailDemo 1>> %logfile% 2>>&1
call :disable_svc diagsvc 1>> %logfile% 2>>&1
call :disable_svc shpamsvc 1>> %logfile% 2>>&1
call :disable_svc TermService 1>> %logfile% 2>>&1
call :disable_svc UmRdpService 1>> %logfile% 2>>&1
call :disable_svc SessionEnv 1>> %logfile% 2>>&1
call :disable_svc TroubleshootingSvc 1>> %logfile% 2>>&1
call :disable_svc OneSyncSvc 1>> %logfile% 2>>&1
call :disable_svc MessagingService 1>> %logfile% 2>>&1
call :disable_svc PimIndexMaintenanceSvc 1>> %logfile% 2>>&1
call :disable_svc UserDataSvc 1>> %logfile% 2>>&1
call :disable_svc UnistoreSvc 1>> %logfile% 2>>&1
call :disable_svc BcastDVRUserService 1>> %logfile% 2>>&1
call :disable_svc Sgrmbroker 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Отключить Software Reporter Tool и сервис обновления в Google Chrome.%clr%[92m
@echo Отключить Software Reporter Tool и сервис обновления в Google Chrome. 1>> %logfile%
set timerStart=!time!
...
if !errorlevel!==1 (
	...
	REM ; Отображать Компьютер на Рабочем столе
	reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel" /v "{20D04FE0-3AEA-1069-A2D8-08002B30309D}" /t REG_DWORD /d "0" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\*\shell\TakeOwn" /v "SubCommands" /t REG_SZ /d "takeown;restown" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Directory\shell\TakeOwn" /v "SubCommands" /t REG_SZ /d "takeownf;restown" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Drive\shell\TakeOwn" /v "SubCommands" /t REG_SZ /d "takeownf" /f 1>> %logfile% 2>>&1
	...
	REM ; Название пункта меню смены владельца для файлов
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\takeown" /v "HasLUAShield" /t REG_SZ /d "" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\takeown" /ve /t REG_SZ /d "Назначить владельцем текущего пользователя" /f 1>> %logfile% 2>>&1
	REM ; Команда смены владельца файлов
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\takeown\command" /ve /t REG_SZ /d "nircmdc elevate cmd.exe /c takeown /f 1>> %logfile% 2>>&1 \"%%1\" && icacls \"%%1\" /grant Администраторы:F /c /l" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\takeown\command" /v "IsolatedCommand" /t REG_SZ /d "nircmdc elevate cmd.exe /c takeown /f 1>> %logfile% 2>>&1 \"%%1\" && icacls \"%%1\" /grant Администраторы:F /c /l" /f 1>> %logfile% 2>>&1
	REM ; Название пункта меню установки владельца TrustedInstaller для файлов и папок
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\restown" /v "HasLUAShield" /t REG_SZ /d "" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\restown" /ve /t REG_SZ /d "Назначить владельцем TrustedInstaller" /f 1>> %logfile% 2>>&1
	REM ; Команда установки владельца TrustedInstaller для файлов и папок
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\restown\command" /ve /t REG_SZ /d "nircmdc elevate cmd.exe /c icacls \"%%1\" /setowner \"NT Service\TrustedInstaller\" /T /C" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\restown\command" /v "IsolatedCommand" /t REG_SZ /d "nircmdc elevate cmd.exe /c icacls \"%%1\" /setowner \"NT Service\TrustedInstaller\" /T /C" /f 1>> %logfile% 2>>&1
	REM ; Название пункта меню смены владельца для папок
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\takeownf" /v "HasLUAShield" /t REG_SZ /d "" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\takeownf" /ve /t REG_SZ /d "Назначить владельцем текущего пользователя" /f 1>> %logfile% 2>>&1
	REM ; Команда смены владельца папок
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\takeownf\command" /ve /t REG_SZ /d "nircmdc elevate cmd.exe /c takeown /f 1>> %logfile% 2>>&1 \"%%1\" /r /d y && icacls \"%%1\" /grant Администраторы:F /t /c /l /q" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\takeownf\command" /v "IsolatedCommand" /t REG_SZ /d "nircmdc elevate cmd.exe /c takeown /f 1>> %logfile% 2>>&1 \"%%1\" /r /d y && icacls \"%%1\" /grant Администраторы:F /t /c /l /q" /f 1>> %logfile% 2>>&1
	REM ; Администрирование в папке Компьютер
	...
	reg add "HKCR\CLSID\{98641F47-8C25-4936-BEE4-C2CE1298969D}\DefaultIcon" /ve /t REG_SZ /d "charmap.exe,0" /f 1>> %logfile% 2>>&1
	reg add "HKCR\CLSID\{98641F47-8C25-4936-BEE4-C2CE1298969D}\Shell\Open\command" /ve /t REG_SZ /d "charmap.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\dllfile\Shell\DLLReg\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller \"regsvr32.exe \"%%1\"\"" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\dllfile\Shell\CancelReg\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller \"regsvr32.exe /u \"%%1\"\"" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\ocxfile\Shell\OCXReg\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller \"regsvr32.exe \"%%1\"\"" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\AllFilesystemObjects\shell\CopyAsPathMenu\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller \"cmd.exe /c \"echo %%1 | clip\"\"" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Directory\shell\DeleteFolderContent\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller \"cmd.exe /c \"cd /d %%1 && del /s /f 1>> %logfile% 2>>&1 /q *.*\"\"" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Drive\shell\CleanMgr\command" /ve /t REG_SZ /d "cleanmgr.exe /d %%1" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Drive\shell\Defrag\command" /ve /t REG_SZ /d "defrag %%1 -v" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Directory\shell\CmdFolder" /v "SubCommands" /t REG_SZ /d "cmd_admin;cmd_user" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Directory\Background\shell\CmdFolder" /v "SubCommands" /t REG_SZ /d "cmd_admin;cmd_user" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Drive\shell\CmdFolder" /v "SubCommands" /t REG_SZ /d "cmd_admin;cmd_user" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\cmd_admin\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller cmd.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Directory\Background\shell\Eject\command" /ve /t REG_EXPAND_SZ /d "devxexec.exe /user:TrustedInstaller \"wscript.exe %SystemRoot%/Tools/EjectFlash/EjectFlash.vbs\"" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Directory\Background\shell\Standart" /v "SubCommands" /t REG_SZ /d "mute;hide;topmost;centerall;closeall;winmanager;screen;search;scanner;webcam;calc;paint;charmap;stikynot;soundrec;snip;psr" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Directory\Background\shell\Admin" /v "SubCommands" /t REG_SZ /d "cmd;regedit;services;rsop;run;taskschd;wf;network;useracc;useracc2;eventvwr;perfmon;relmon;trouble" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\deviceproperties\command" /ve /t REG_SZ /d "nircmdc.exe elevatecmd runassystem mmc devmgmt.msc" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\diskmng\command" /ve /t REG_SZ /d "nircmdc.exe elevatecmd runassystem mmc diskmgmt.msc" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\services\command" /ve /t REG_SZ /d "nircmdc.exe elevatecmd runassystem mmc services.msc" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\gpedit\command" /ve /t REG_SZ /d "nircmdc.exe elevatecmd runassystem mmc gpedit.msc" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\taskschd\command" /ve /t REG_SZ /d "nircmdc.exe elevatecmd runassystem mmc taskschd.msc /s" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\eventvwr\command" /ve /t REG_SZ /d "nircmdc.exe elevatecmd runassystem mmc eventvwr.msc /s" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\emptyclip\command" /ve /t REG_SZ /d "nircmdc.exe clipboard clear" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\cleardsk\command" /ve /t REG_SZ /d "hidcon.exe cmd.exe /c cleanmgr /sageset:1 & cleanmgr /sagerun:1" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\reducemem" /v "Icon" /t REG_EXPAND_SZ /d "%SystemRoot%\Tools\ReduceMemory.exe" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\reducemem\command" /ve /t REG_EXPAND_SZ /d "devxexec.exe /user:TrustedInstaller %SystemRoot%/Tools/ReduceMemory.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\lockoffmon\command" /ve /t REG_EXPAND_SZ /d "devxexec.exe /user:TrustedInstaller %SystemRoot%/Tools/MonitorOff.bat" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\cancelshutdown\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller shutdown.exe -a" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\mute\command" /ve /t REG_SZ /d "nircmdc.exe mutesysvolume 2" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\topmost" /v "Icon" /t REG_EXPAND_SZ /d "%SystemRoot%/Tools/TopMost.exe" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\topmost\command" /ve /t REG_EXPAND_SZ /d "%SystemRoot%/Tools/TopMost.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\centerall\command" /ve /t REG_SZ /d "nircmdc.exe win center alltop" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\closeall\command" /ve /t REG_SZ /d "nircmdc.exe win close class CabinetWClass" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\stikynot" /ve /t REG_SZ /d "Записки" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\stikynot" /v "Icon" /t REG_SZ /d "StikyNot.exe" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\stikynot\command" /ve /t REG_SZ /d "StikyNot.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\updtime" /v "Icon" /t REG_EXPAND_SZ /d "%SystemRoot%/Tools/UpdateTime.exe" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\updtime\command" /ve /t REG_EXPAND_SZ /d "%SystemRoot%/Tools/UpdateTime.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\fixbrowsers\command" /ve /t REG_EXPAND_SZ /d "devxexec.exe /user:TrustedInstaller %SystemRoot%/Tools/f 1>> %logfile% 2>>&1ixBrowsers.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\fixprints" /v "Icon" /t REG_EXPAND_SZ /d "%SystemRoot%/Tools/f 1>> %logfile% 2>>&1ixPrintS.exe" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\fixprints\command" /ve /t REG_EXPAND_SZ /d "devxexec.exe /user:TrustedInstaller %SystemRoot%/Tools/f 1>> %logfile% 2>>&1ixPrintS.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\drvcln_x86\command" /ve /t REG_EXPAND_SZ /d "devxexec.exe /user:TrustedInstaller \"%SystemRoot%/Tools/DriveCleanup/DriveCleanup_x86.exe -n\"" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\drvcln_x64\command" /ve /t REG_EXPAND_SZ /d "devxexec.exe /user:TrustedInstaller \"%SystemRoot%/Tools/DriveCleanup/DriveCleanup_x64.exe -n\"" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\devmgr" /v "Icon" /t REG_SZ /d "devmgr.dll,4" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\rstrui\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller rstrui.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\cmd\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller cmd.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\fixwin" /v "Icon" /t REG_EXPAND_SZ /d "%SystemRoot%/Tools/f 1>> %logfile% 2>>&1ixWin.exe" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\fixwin\command" /ve /t REG_EXPAND_SZ /d "%SystemRoot%\Tools\FixWin.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\wu" /v "Icon" /t REG_SZ /d "wucltux.dll" /f 1>> %logfile% 2>>&1
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\wu\command" /ve /t REG_SZ /d "wuapp.exe" /f 1>> %logfile% 2>>&1
	...
	reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\run\command" /ve /t REG_SZ /d "devxexec.exe /user:TrustedInstaller \"explorer.exe shell:::{2559A1F3-21D7-11D4-BDAF-00C04F60B9F0}\"" /f 1>> %logfile% 2>>&1
	set timerEnd=!time!
	call :timer
	@echo ОК %clr%[93m[%clr%[91m!mins!%clr%[0m минут%clr%[93m %clr%[91m!secs!%clr%[0m секунд%clr%[93m]%clr%[92m
)
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Программа для отключения ленты из проводника.%clr%[92m %clr%[7;31mПредупреждение: после применения изменений не нажимайте кнопку Yes для дальнейшего применения скрипта.%clr%[0m%clr%[36m%clr%[92m
...
goto :eof
:demand_svc_sudo
%SystemUser% cmd.exe /c sc config "%~1" start= demand ^& sc stop "%~1" 1>> %logfile% 2>>&1
goto :eof
:disable_svc
sc stop "%~1" 1>> %logfile% 2>>&1
...
goto :eof
:disable_svc_sudo
%SystemUser% cmd.exe /c sc config "%~1" start= disabled ^& sc stop "%~1" 1>> %logfile% 2>>&1
goto :eof
:disable_svc_hard
%SystemUser% cmd.exe /c %acl% /process "%~1" /setowner=S-1-5-32-544 /grant=S-1-5-32-544=F /grant=S-1-5-18=F 1>> %logfile% 2>>&1
%SystemUser% cmd.exe /c %acl% /subkeyreg "HKLM\SYSTEM\CurrentControlSet\Services\%~1" /setowner=S-1-5-32-544 /grant=S-1-5-32-544=F /grant=S-1-5-18=F 1>> %logfile% 2>>&1
%SystemUser% cmd.exe /c sc config "%~1" start= disabled ^& sc stop "%~1" 1>> %logfile% 2>>&1
goto :eof
:acl_file
%SystemUser% cmd.exe /c %acl% /file "%~1" /setowner=S-1-5-32-544 /grant=S-1-5-32-544=F /grant=S-1-5-18=F 1>> %logfile% 2>>&1
goto :eof
:acl_folders
%SystemUser% cmd.exe /c %acl% /subdirectories "%~1" /setowner=S-1-5-32-544 /grant=S-1-5-32-544=F /grant=S-1-5-18=F 1>> %logfile% 2>>&1
goto :eof
:acl_registry
%SystemUser% cmd.exe /c %acl% /subkeyreg "%~1" /setowner=S-1-5-32-544 /grant=S-1-5-32-544=F /grant=S-1-5-18=F 1>> %logfile% 2>>&1
goto :eof
:disable_task
schtasks /change /disable /tn "%~1" 1>> %logfile% 2>>&1
goto :eof
:trusted
::tasklist /fo table /nh /fi "imagename eq trustedinstaller.exe" >nul | find /i "trustedinstaller.exe" >nul || (net start trustedinstaller >nul)
::%~dp0devxexec.exe /user:TrustedInstaller "%~1"
goto :eof
:remove_uwp
powershell "Get-AppxPackage *%~1* -AllUsers | Remove-AppxPackage -AllUsers -WarningAction SilentlyContinue"
goto :eof
:remove_uwp_hard
