...
setx NUGET_XMLDOC_MODE skip 1>> %logfile% 2>>&1
...
if !errorlevel!==1 (
	...
	if exist "%SystemDrive%\Windows.old" (
		call :acl_folders "%SystemDrive%\Windows.old"
	...
	%SystemRoot%\system32\cmd.exe /c "wevtutil sl Microsoft-Windows-LiveId/Operational /ca:O:BAG:SYD:(A;;0x1;;;SY)(A;;0x5;;;BA)(A;;0x1;;;LA)" 1>> %logfile% 2>>&1
	for /f "tokens=*" %%i in ('wevtutil.exe el') do (wevtutil.exe cl %1 "%%i" 1>> %logfile% 2>>&1)
	net user defaultuser0 /delete 1>> %logfile% 2>>&1
	net start DPS %logfile% 2>>&1
...
reg add "HKLM\SOFTWARE\Microsoft\Windows\Windows Error Reporting\Consent" /v "DefaultConsent" /t REG_DWORD /d "0" /f /f 1>> %logfile% 2>>&1
reg add "HKLM\SOFTWARE\Microsoft\Windows\Windows Error Reporting\Consent" /v "DefaultOverrideBehavior" /t REG_DWORD /d "1" /f /f 1>> %logfile% 2>>&1
...
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" /v "SystemResponsiveness" /t REG_DWORD /d "10" /f 1>> %logfile% 2>>&1
...
call :acl_file "%SystemRoot%\System32\mcupdate_AuthenticAMD.dll"
call :acl_file "%SystemRoot%\System32\mcupdate_GenuineIntel.dll"
...
if !errorlevel!==1 (
	...
	::curl -fSLo "%SystemRoot%\System32\drivers\etc\hosts" "http://sbc.io/hosts/alternates/fakenews-gambling-porn-social/hosts"
...
reg delete "HKCR\Launcher.AllAppsDesktopApplication\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
reg delete "HKCR\Launcher.Computer\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
reg delete "HKCR\Launcher.DesktopPackagedApplication\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
reg delete "HKCR\Launcher.DualModeApplication\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
reg delete "HKCR\Launcher.ImmersiveApplication\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
reg delete "HKCR\Launcher.SystemSettings\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
...
reg delete "HKLM\SOFTWARE\Classes\Launcher.AllAppsDesktopApplication\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
reg delete "HKLM\SOFTWARE\Classes\Launcher.Computer\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
reg delete "HKLM\SOFTWARE\Classes\Launcher.DesktopPackagedApplication\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
...
reg delete "HKLM\SOFTWARE\Classes\Launcher.ImmersiveApplication\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
reg delete "HKLM\SOFTWARE\Classes\Launcher.SystemSettings\shellex\ContextMenuHandlers\PintoStartScreen" /f 1>> %logfile% 2>>&1
...
reg delete "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\Windows.PinToHome" /f 1>> %logfile% 2>>&1
reg delete "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\CommandStore\shell\Windows.pintostartscreen" /f 1>> %logfile% 2>>&1
...
@echo Загрузить и установить NET Framework 3.5 версии. 1>> %logfile%
set timerStart=!time!
:: curl -fSLo "%~dp0dotnetcoresdk.exe" "https://download.visualstudio.microsoft.com/download/pr/674a9f7d-862e-4f92-91b6-f1cf3fed03ce/e07db4de77ada8da2c23261dfe9db138/dotnet-sdk-5.0.103-win-x64.exe"
...
:: curl -fSLo "%~dp0microsoft-windows-netfx3.zip" "https://dotnetbinaries.blob.core.windows.net/dockerassets/microsoft-windows-netfx3-1909.zip"
...
%PS% "Expand-Archive -Force "%~dp0microsoft-windows-netfx3.zip" "%~dp0" -wa SilentlyContinue" 1>> %logfile% 2>>&1
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Disable Customer Experience Improvement (CEIP/SQM).%clr%[92m
@echo Disable Customer Experience Improvement (CEIP/SQM). 1>> %logfile%
set timerStart=!time!
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Disable license telemetry.%clr%[92m
@echo Disable license telemetry. 1>> %logfile%
set timerStart=!time!
...
if !errorlevel!==1 (
	...
	reg add "HKCR\Directory\Background\shell" /ve /t REG_SZ /d "MyComp;ShowDesk;Eject;CleanTools,Refresh,CmdFolder,Standart,System,Admin,Advanced,ReIcon,PowerMenu,Display,Gadgets,Personalize" /f 1>> %logfile% 2>>&1
	...
	reg add "HKCR\Directory\Background\shell\ShowDesk" /v "MUIVerb" /t REG_SZ /d "Показать рабочий стол" /f 1>> %logfile% 2>>&1
	reg add "HKCR\Directory\Background\shell\ShowDesk" /v "Icon" /t REG_SZ /d "explorer.exe,16" /f 1>> %logfile% 2>>&1
...
set timerStart=!time!
%PS% "%~dp0AppGraphicsPerformance.ps1" -wa SilentlyContinue 1>> %logfile% 2>>&1
set timerEnd=!time!
call :timer
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
...
@echo ОК %clr%[93m[%clr%[91m!totalsecs!.!ms!%clr%[0m секунд%clr%[93m]%clr%[92m
echo. 1>> %logfile%
@echo %clr%[36m Программа для настройки задержками прерываний на устройствах (MSI Mode).%clr%[92m %clr%[7;31mПредупреждение:%clr%[0m%clr%[36m%clr%[92m применяйте параметры только для устройств с поддержкой msi.
...
if !errorlevel!==1 (
	...
	:: curl -fSLo "%~dp0FSClient\FSClient.UWP.cer" "https://fsclient.github.io/fs/FSClient.UWP/FSClient.UWP.cer"
	:: curl -fSLo "%~dp0FSClient\FSClient.UWP.appxbundle" "https://fsclient.github.io/fs/FSClient.UWP/FSClient.UWP.appxbundle"
...
@echo %clr%[36m Скрыть все водяные напоминания об активации, тестовом, безопасном режиме и другие?%clr%[92m %clr%[7;31mПредупреждение:%clr%[0m%clr%[36m%clr%[92m не закрывайте окна выполнения и не нажимайте на кнопки подтверждения.%clr%[92m
choice /c yn /n /t %autoChoose% /d y /m %keySelY%
if !errorlevel!==1 (
...
if !errorlevel!==1 (
	...
	set timerStart=!time!
	xcopy "%~dp0EmptyStandbyList.exe" "%SystemRoot%\Tools\EmptyStandbyList.exe" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	xcopy "%~dp0MSI_util_v3.exe" "%SystemRoot%\Tools\MSI_util_v3.exe" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	xcopy "%~dp0..\Tools\nvidiaProfileInspector.exe" "%SystemRoot%\Tools\nvidiaProfileInspector.exe" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	echo F|xcopy "%~dp0AppGraphicsPerformance.ps1" "%SystemRoot%\Tools\AppGraphicsPerformance.ps1" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	xcopy "%~dp0..\Cleanmgr+" "%SystemRoot%\Tools\Cleanmgr+\*.*" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	xcopy "%~dp0..\Tools\intPolicy_%arch%.exe" "%SystemRoot%\Tools\intPolicy_%arch%.exe" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	xcopy "%~dp0..\NV RGBFullRangeToggle\NV_RGBFullRangeToggle.exe" "%SystemRoot%\Tools\NV_RGBFullRangeToggle.exe" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	xcopy "%~dp0..\NVCleanstall\NVCleanstall_1.10.0.exe" "%SystemRoot%\Tools\NVCleanstall.exe" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	...
	xcopy "%~dp0..\TimerTool\TimerTool.exe" "%SystemRoot%\Tools\TimerTool.exe" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	xcopy "%~dp0..\Tools\REAL.exe" "%SystemRoot%\Tools\REAL.exe" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	xcopy "%~dp0..\vibranceGUI 2.3.1.1\vibranceGUI.exe" "%SystemRoot%\Tools\vibranceGUI.exe" /e /c /i /q /h /r /y 1>> %logfile% 2>>&1
	start "" /wait %~dp0\..\Installers\CompatibilityManager.exe
	...
	nircmdc shortcut "%SystemRoot%\Tools\intPolicy_%arch%.exe" "~$folder.common_desktop$\Дополнительные ярлыки" "Настройка политики сродства прерывания драйверов" "" "%SystemRoot%\System32\imageres.dll" 151
	...
	nircmdc shortcut "%SystemRoot%\Tools\QuickCpu 3.5.0.0\QuickCPU.exe" "~$folder.common_desktop$\Дополнительные ярлыки" "Тонкая настройка и мониторинг производительности процессора" "" "%SystemRoot%\Tools\QuickCpu 3.5.0.0\QuickCPU.exe"
	...
	nircmdc shortcut "%SystemRoot%\Tools\REAL.exe" "~$folder.common_desktop$\Дополнительные ярлыки" "Уменьшение задержки звука на устройствах воспроизведения (после запуска приложение необходимо свернуть)" "--tray" "%SystemRoot%\Tools\REAL.exe"
