...
begin
  ...
  { ShellExecuteEx doesn't provide a possibility to bypass Image File Execution
    Options. So we rely on the interception Ч it will show the correct UAC, but
    launch ourselves with high privileges instead. In case of Ask.exe, we need
    to force it to accept "Yes" without asking. When process will exit Ч its
...
begin
  if IsElevated then
  begin
    OutputDebugString('ќбнаружен бесконечный цикл: процесс с повышенными правами ' +
      'запросил повышение прав. ¬ы пытаетесь запустить программу UIAccess?');
